...
- AI 配置支持启用/禁用、编辑和删除；禁用配置不会出现在普通用户选择列表中。
- 管理员查询模型后可按聊天模型、生图模型和视频模型分类管理；分类支持自动识别、手动调整、重复归类和独立保存。
- AI 聊天支持在同一对话中切换模型分类，模型下拉列表按分类实时更新，“应用模型”会保存当前会话选择。
- AI 会话增加生图/生视频任务状态和单会话并发保护；当前未接入具体厂商生成协议时会显示明确提示。
- 页面懒加载和接口请求提供“正在加载”“正在保存”“正在删除”等操作提示，AI 配置弹窗错误仅在弹窗内部显示。
- 当前版本为 `0.5.3`。数据库迁移会自动执行，SQLite 和 PostgreSQL 均适用。
