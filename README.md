...
系统根路径 `http://服务器IP:10000/` 默认显示公开 Home 主页。主页显示当前站点名称、站点副标题和站点 Logo，并提供登录、注册、GitHub 项目和项目文档入口。当前默认站点名称为“HG-工具小屋”，管理员可以在“基础系统设置-站点信息”中修改，主页会自动读取最新配置。

认证页面使用独立路径：

- `/login`：登录；系统尚未初始化时，此页面显示初始化管理员账号表单。
- `/register`：注册账号；是否允许注册由管理员配置控制。
- `/reset-password`：重置密码。

已登录用户访问根路径时仍显示公开主页，右上角会显示当前账号，并提供进入系统和退出登录按钮。主页支持中文/英文切换和白天/黑夜模式，选择结果保存到浏览器本地存储。主页底部提供关于项目、文档、相关项目和友情链接栏目，当前链接仅作为页面占位，不执行跳转。

## 主要功能

- **数据分析**：导入视频号后台 CSV、逐视频表格或截图 OCR；支持去重、修订记录、日期区间查询、趋势图表、视频贡献和 AI 分析建议。
- **视频号账号**：每个用户可维护多个视频号账号，分析数据按账号隔离；管理员可统一管理所有账号。所有用户可编辑、删除和启用/禁用自己主账号下的视频号，禁用账号不会出现在选择列表中，删除会一并清理关联数据。
...
- **用户与权限**：支持管理员、超级会员、会员和普通用户等级；用户资料、视频号账号、AI 会话和使用次数按用户持久化并隔离。管理员使用“注册用户管理”维护注册用户，并可按最近页面活动查看在线/不在线状态。
- **系统安全**：支持邮箱注册与密码重置、Cloudflare Turnstile、人机验证开关、审计日志、加密 API Key 和加密备份。
- **菜单显示管理**：管理员可在系统设置中配置普通用户侧边栏的可配置菜单；支持一级/二级菜单的显示、隐藏、全选、全隐藏和半选状态，不可配置的固定菜单会明确禁用。配置变化时，在线普通用户会收到刷新提示，也可暂不更新并在下次登录时加载。
- **菜单快捷方式**：管理员可为二级菜单设置快捷方式。管理员顶部快捷区域显示全部已设置快捷方式，普通用户只显示自己有权限访问的快捷方式；快捷按钮复用对应菜单图标，并随菜单自定义名称更新。
