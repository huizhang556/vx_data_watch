...
| 内存 | 2 GB（建议启用至少 2 GB swap） | 4 GB 或更多 |
...
OCR 资源要求：截图识别会加载 RapidOCR/ONNX Runtime，并会随图片分辨率和并发图片数量增加内存占用。最低建议为 2 核 CPU、2 GB 内存加 2 GB swap；稳定处理多张或高分辨率截图建议使用 4 GB 以上内存。小内存服务器应压缩截图、单张上传，并避免同时处理多张图片。
...
| 变量 | 用途 | 默认值 |
| --- | --- | --- |
| `VX_IMAGE` | 应用镜像及版本 | `docker.io/litehub/vx-data-watch:0.3.6` |
