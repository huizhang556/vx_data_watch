...
def test_version_payload_offers_all_other_semver_versions(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    assert payload["current_version"] == "0.5.2"
    assert [row["version"] for row in payload["versions"]] == ["0.5.1", "0.4.3", "0.4.2", "0.3.4", "0.3.3", "0.3.2", "0.3.0", "0.2.1"]
...
def test_updater_pulls_replaces_and_persists_image(tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    assert engine.calls == [
        ...
        ("replace", "vx-data-watch", "app", "litehub/vx-data-watch", "0.3.2"),
    ]
    assert "VX_IMAGE=docker.io/litehub/vx-data-watch:0.3.2" in env_file.read_text()
