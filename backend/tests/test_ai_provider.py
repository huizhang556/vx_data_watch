...
def _payload(model: str = "test-model") -> dict[str, Any]:
    return {
        "name": "测试 AI",
...
def test_provider_is_saved_only_after_successful_test(
    ...
    saved = client.post("/api/ai/provider/test-and-save", headers=auth, json=_payload())
    assert saved.status_code == 200, saved.text
    assert saved.json()["result"] == "连接成功"
    assert client.get("/api/ai/provider").json()["model"] == "test-model"
...
def test_draft_models_and_test_do_not_change_saved_provider(
    ...
    rejected = client.post(
        "/api/ai/provider/test-and-save", headers=auth, json=_payload("bad-model")
    )
