...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  return <main className="auth-page"><section className="auth-panel"><div className="brand-mark">VX</div><Typography.Title level={2}>视频号数据分析</Typography.Title><Typography.Paragraph type="secondary">{isSetup ? '创建首个本地管理员账号' : mode === 'login' ? '使用账号进入数据工作台' : mode === 'register' ? '使用邮箱验证码创建账号' : '使用注册邮箱重置密码'}</Typography.Paragraph>{error && <Alert type="error" showIcon message={error} />}
    <Form form={form} layout="vertical" onFinish={submit} requiredMark={false} key={mode}>{(mode === 'login' || mode === 'register') && <Form.Item name="username" label="用户名" rules={[{ required: true, message: '请输入用户名' }, { min: 3, message: '用户名至少需要 3 个字符' }, usernameRule]}><Input prefix={<UserRound size={17} />} autoComplete="username" onBlur={(event) => syncField('username', event.currentTarget.value)} /></Form.Item>}{(mode === 'register' || mode === 'reset') && <Form.Item name="email" label="注册邮箱" rules={[{ required: true, type: 'email', message: '请输入有效邮箱' }]}><Input prefix={<Mail size={17} />} autoComplete="email" onChange={(event) => syncField('email', event.target.value)} onBlur={(event) => syncField('email', event.currentTarget.value)} /></Form.Item>}{mode !== 'login' && <Form.Item name="code" label="邮箱验证码" rules={[{ required: true, message: '请输入 6 位数字验证码' }, { len: 6, pattern: /^\d{6}$/, message: '验证码必须是 6 位数字' }]}><Input.Search enterButton="发送验证码" onSearch={() => { void sendCode(form.getFieldValue('email')) }} /></Form.Item>}<Form.Item name="password" label={mode === 'login' ? '密码' : '新密码'} rules={[{ required: true, message: '请输入密码' }, ...(mode === 'login' ? [] : [{ min: 10, message: '密码至少需要 10 个字符' }, passwordRule])]}><Input.Password prefix={<LockKeyhole size={17} />} autoComplete={mode === 'login' ? 'current-password' : 'new-password'} onBlur={(event) => syncField('password', event.currentTarget.value)} /></Form.Item><Captcha enabled={config.captcha_enabled} siteKey={config.captcha_site_key} onChange={setCaptcha} /><Button block type="primary" htmlType="submit" size="large" loading={busy}>{isSetup ? '初始化系统' : mode === 'login' ? '登录' : mode === 'register' ? '注册并登录' : '重置密码并登录'}</Button></Form>
