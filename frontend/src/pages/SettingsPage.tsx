...
interface AuthSettings {
  ...
  smtp_ssl: boolean;
  verification_code_minutes: number;
...
const usernameRules = [
  ...
  {
    pattern: /^[A-Za-z0-9_.-]+$/,
    message: "用户名只能包含英文、数字、下划线、点和短横线",
  },
];
const passwordRules = [
  { required: true, message: "请输入密码" },
  { min: 10, max: 200, message: "密码长度应为 10-200 个字符" },
  {
    pattern: /^(?=.*[A-Za-z])(?=.*\d).+$/,
    message: "密码至少 10 位，并同时包含字母和数字",
  },
];
...
type SettingsSection = "settings" | "users" | "accounts" | "local";
export default function SettingsPage({
  ...
  const loadUsers = () =>
    user.role === "admin"
      ? api<LocalUser[]>("/api/users").then(setUsers)
      : Promise.resolve();
  ...
  const saveAuthSettings = async (
    ...
    notify = true,
    successMessage = "邮箱与注册配置已保存",
  ) => {
    setLoading(true);
    try {
      const saved = await api<AuthSettings>("/api/settings/auth", {
        method: "PUT",
      ...
      if (notify)
        message.error(
          cause instanceof Error ? cause.message : "保存邮箱配置失败",
        );
      throw cause;
    } finally {
      setLoading(false);
  ...
  const testAuthSMTP = async () => {
    ...
    try {
      ...
      await saveAuthSettings(
        { ...authForm.getFieldsValue(), ...smtpValues },
        false,
      );
      await api("/api/settings/auth/test", {
        method: "POST",
      ...
      message.error(
        cause instanceof Error ? cause.message : "测试邮件发送失败",
      );
    } finally {
      setLoading(false);
    }
  ...
  const updateUser = async (values: {
    ...
    try {
      ...
      message.error(cause instanceof Error ? cause.message : "保存用户失败");
    } finally {
      setLoading(false);
    }
  };
  const deleteUser = async (row: LocalUser) => {
    try {
      ...
      message.error(cause instanceof Error ? cause.message : "删除用户失败");
    }
  };
  const avatarUpload = (form: ReturnType<typeof Form.useForm>[0]) => (
    <Form.Item name="avatar" label="头像">
      <Upload
        ...
        beforeUpload={(file) => {
          ...
          reader.onload = () => form.setFieldValue("avatar", reader.result);
          reader.readAsDataURL(file);
          return false;
        }}
      >
        <Button icon={<ImagePlus size={16} />}>选择头像</Button>
      </Upload>
    </Form.Item>
  );
  return (
    <div className="page">
        ...
        <section className="section-band auth-settings-section">
          ...
          {user.role === "admin" && (
            ...
            >
              ...
              >
                <InputNumber min={1} max={65535} style={{ width: 190 }} />
              </Form.Item>
              ...
              <Form.Item
                name="smtp_password"
                label={
                  authSettings?.smtp_password_set
                    ? "SMTP 密码（留空保持原密码）"
                    : "SMTP 密码"
                }
                rules={
              ...
              >
                <Select
                  options={[
                    { value: true, label: "STARTTLS（587）" },
                    { value: false, label: "SSL（465）" },
                  ]}
                />
              </Form.Item>
              <Form.Item name="smtp_ssl" label="SSL 连接（465）">
                <Select
                  options={[
                    { value: true, label: "开启" },
              ...
              <Form.Item
                name="verification_code_minutes"
                label="验证码有效期（分钟）"
                rules={[
                  { required: true, message: "请输入验证码有效期" },
                  {
                    type: "number",
                    min: 1,
                    max: 60,
                    message: "有效期范围为 1-60 分钟",
                  },
                ]}
              >
                <InputNumber min={1} max={60} style={{ width: 190 }} />
              </Form.Item>
              <div className="settings-actions">
                ...
                <Input
                  ...
                  placeholder="测试收件邮箱"
                />
                <Button onClick={() => void testAuthSMTP()} loading={loading}>
                  发送测试邮件
                </Button>
              </div>
            </Form>
          ...
          {user.role === "admin" && (
            ...
            >
              ...
              <Form.Item name="captcha_provider" label="验证服务商">
                <Select
                  options={[
              ...
              <Form.Item
                ...
                label="站点密钥"
                rules={[
                  { required: true, message: "请输入 Turnstile 站点密钥" },
                ]}
              >
                <Input />
              </Form.Item>
              <Form.Item
                ...
                label={
                  authSettings?.captcha_secret_key_set
                    ? "私密密钥（留空保持原密钥）"
                    : "私密密钥"
                }
                rules={
                  authSettings?.captcha_secret_key_set
                    ? []
                    : [{ required: true, message: "请输入 Turnstile 私密密钥" }]
                }
              >
                <Input.Password autoComplete="new-password" />
              </Form.Item>
        ...
        <section className="section-band">
          ...
          {!accounts.length ? (
            <Empty
              ...
              description="还没有视频号账号"
            />
          ) : (
            <div className="account-list">
              {accounts.map((account) => (
                <article key={account.id}>
                  <div className="account-icon">号</div>
                  <div>
                  ...
                  render: (_, row) => (
                    <>
                      <Button
                        ...
                        onClick={() => {
                          setEditingUser(row);
                          editUserForm.setFieldsValue(row);
                          setEditUserOpen(true);
                        }}
                      />
                      <Popconfirm
                        title="删除用户"
                        description={`确定删除 ${row.username} 吗？`}
                        okText="删除"
                        cancelText="取消"
                        okButtonProps={{ danger: true }}
                        onConfirm={() => void deleteUser(row)}
                      >
                        <Button
                          type="text"
                          danger
      ...
      <Modal
        ...
        destroyOnHidden
      >
        <Form layout="vertical" onFinish={createAccount} requiredMark={false}>
      ...
      >
        <Form
          ...
          layout="vertical"
          onFinish={createUser}
          initialValues={{ role: "viewer", level: 1 }}
          requiredMark={false}
        >
          ...
          <Form.Item
            name="level"
            label="用户等级"
            extra="按用户等级限制每日功能使用次数"
          >
            <Select
              style={{ width: 130 }}
              options={[
                { value: 0, label: "0（普通用户）" },
                { value: 1, label: "1（会员用户）" },
                { value: 2, label: "2（超级会员）" },
                { value: 3, label: "3（管理员）" },
              ]}
            />
          </Form.Item>
          {avatarUpload(userForm)}
          <Button block type="primary" htmlType="submit" loading={loading}>
            创建用户
          </Button>
        </Form>
      ...
      >
        ...
        >
          ...
          <Form.Item name="is_active" label="账号状态">
            <Select
              options={[
                { value: true, label: "启用" },
