...
test('initializes, imports seven-day CSV, and renders desktop/mobile', async ({ page }) => {
  await page.route('**/api/system/versions', (route) => route.fulfill({ json: { current_version: '0.3.6', latest_version: '0.3.6', versions: [{ version: '0.3.6', published_at: '2026-08-19T00:00:00Z' }], repository: 'litehub/vx-data-watch', update_supported: true, deployment: 'docker' } }))
  await page.route('**/api/system/update-status', (route) => route.fulfill({ json: { state: 'idle', current_version: '0.3.6' } }))
  await page.goto('/')
  ...
  await expect(page.getByText('v0.3.6').first()).toBeVisible()
